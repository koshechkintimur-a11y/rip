...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const d = await apiGet<{ season: { number: number; ends_at: string } }>('/api/state');
  ...
  const steps = [
    ...
    <div key="1" className="text-center px-6">
      <h1 className="text-5xl font-black tracking-tighter mb-6">RIP</h1>
      <p className="text-lg text-rip-text/90 leading-relaxed">
        Социальная сеть,
        <br />
        которая <span className="text-rip-blood font-semibold">не живёт вечно</span>.
      </p>
      <p className="mt-4 text-sm text-rip-dim">
        Каждый сезон длится 7 дней.
      </p>
    </div>,
    // Экран 2: цикл
    <div key="2" className="text-center px-6">
      <h2 className="text-2xl font-black tracking-widest mb-6">
        ПИШИ.<br />ОТВЕЧАЙ.<br /><span className="text-rip-blood">ВЫЖИВАЙ.</span>
      </h2>
      <div className="bg-rip-panel border border-rip-line rounded-lg p-3 text-left mb-4">
        ...
        <div className="mt-1.5 flex gap-3 text-xs text-rip-dim">
          <span>↳ 4</span><span>💀 8</span><span className="text-rip-gold">⭐ 3</span>
        </div>
      </div>
      <p className="text-sm text-rip-text/85 leading-relaxed">
        Твои сообщения могут остаться в истории,
        пережить сезон, стать легендарными.
        <br />
        <span className="text-rip-blood">А могут просто умереть.</span>
      </p>
    </div>,
    // Экран 3: внимание
    <div key="3" className="text-center px-6">
      <h2 className="text-2xl font-black tracking-widest mb-6">
        ХОЧЕШЬ, ЧТОБЫ ТЕБЯ <span className="text-rip-warn">УВИДЕЛИ?</span>
      </h2>
      <div className="bg-rip-panel border border-rip-warn/40 rounded-lg p-3 mb-4 overflow-hidden">
        <p className="text-[10px] tracking-widest text-rip-warn mb-1">⚡ ВНИМАНИЕ</p>
        <div className="flex gap-2 animate-marquee whitespace-nowrap">
          <span className="text-sm shrink-0">@max: КТО НЕ СПИТ?</span>
          <span className="text-sm shrink-0 text-rip-warn">@anna: Я КУПИЛА 5 МЕСТ</span>
          <span className="text-sm shrink-0">@oleg: кто из Перми?</span>
        </div>
      </div>
      <p className="text-sm text-rip-text/85 leading-relaxed">
        Покупай место в ленте внимания.
        Пиши. Поднимай. <span className="text-rip-warn">Перебивай чужие крики.</span>
      </p>
    </div>,
    // Экран 4: countdown + вход
    <div key="4" className="text-center px-6">
      <h2 className="text-xl font-black tracking-widest mb-2">СЕЗОН {seasonNum ?? 1}</h2>
      <p className="text-[11px] text-rip-dim tracking-widest mb-3">ОСТАЛОСЬ</p>
      <div className="font-mono text-3xl font-bold text-rip-blood mb-6 leading-tight">
